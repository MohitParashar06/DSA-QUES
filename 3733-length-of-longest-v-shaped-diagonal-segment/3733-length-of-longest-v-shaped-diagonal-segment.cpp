...
public:
    ...
    int f(int i, int j, int dr, int cnt,vector<vector<int>>& grid, vector<vector<vector<vector<int>>>> &dp){
        ...
        if(grid[i][j] == 2){
            if(dr == 1){
                if(i-1 >= 0 && j-1 >= 0 && grid[i-1][j-1] == 0){
                    ans = max(ans,f(i-1,j-1,dr,cnt,grid,dp)+1);
                }
                if(i-1 >= 0 && j+1 < m && grid[i-1][j+1] == 0){
                    ans = max(ans,f(i-1,j+1,2,cnt+1,grid,dp)+1);
                }
            }
            if(dr == 2){
                if(i-1 >= 0 && j+1 < m && grid[i-1][j+1] == 0){
                ...
                if(i+1 < n && j+1 < m && grid[i+1][j+1] == 0){
                    ans = max(ans,f(i+1,j+1, 4,cnt+1,grid,dp)+1);
                }
            }
            if(dr == 4){
                if(i+1 < n && j+1 < m && grid[i+1][j+1] == 0){
                ...
                if(i+1 < n && j-1 >= 0 && grid[i+1][j-1] == 0){
                    ans = max(ans,f(i+1,j-1,3,cnt+1,grid,dp)+1);
                }
            }
            if(dr == 3){
                if(i+1 < n && j-1 >= 0 && grid[i+1][j-1] == 0){
                ...
                if(i-1 >= 0 && j-1 >= 0 && grid[i-1][j-1] == 0){
                    ans = max(ans, f(i-1,j-1,1,cnt+1,grid,dp)+1);
                }
            }
        }else{
            if(dr == 1){
    ...
    vector<vector<vector<vector<int>>>> &dp){
        ...
        // cout << i << " " << j << endl;
        if(i-1 >= 0 && j-1 >= 0 && grid[i-1][j-1] == 2)
            ans = max(ans,f(i-1,j-1,1,1,grid,dp)+1);
        if(i-1 >= 0 && j+1 < m && grid[i-1][j+1] == 2)
            ans = max(ans,f(i-1,j+1,2,1,grid,dp)+1);
        if(i+1 < n && j-1 >= 0 && grid[i+1][j-1] == 2)
            ans = max(ans,f(i+1,j-1,3,1,grid,dp)+1);
        if(i+1 < n && j+1 < m && grid[i+1][j+1] == 2)
            ans = max(ans,f(i+1,j+1,4,1,grid,dp)+1);
            // cout << i << " " << j << " " << ans << endl;
        return ans;
    }
